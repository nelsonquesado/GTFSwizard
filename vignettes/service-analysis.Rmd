...
on the same date, and large feeds can contain many IDs that represent the same
set of active dates. GTFSwizard groups identical active-date sets into
`service_pattern` values.

```{r}
get_servicepattern(gtfs)
```

`pattern_frequency` is the number of dates represented by a pattern. The most
